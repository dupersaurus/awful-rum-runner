...
public class DayAndNightControl : MonoBehaviour {
	...
	public float fogMax = 0.08f;

	public Color fogMinColor;
	public Color fogMaxColor;

	public AnimationCurve fogCurve;

	public float ambientMin = 0;
	public float ambientMax = 1;
	...
	void UpdateLight()
	{
		...
		directionalLight.transform.rotation = Quaternion.Euler ((currentTime * 360f) - 90, -70, 0);
		...
		RenderSettings.ambientIntensity = ambientMin + ambientCurve.Evaluate(currentTime) * (ambientMax - ambientMin);
		RenderSettings.fogDensity = fogMin + fogCurve.Evaluate(currentTime) * (fogMax - fogMin);
		RenderSettings.fogColor = Color.Lerp(fogMinColor, fogMaxColor, ambientCurve.Evaluate(currentTime));
	}

	public float lightLevel {
		get { return ambientCurve.Evaluate(currentTime); }
	}
